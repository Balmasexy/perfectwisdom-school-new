import { pgTable, uuid, varchar, text, timestamp, date, integer, decimal, boolean, pgEnum, uniqueIndex, index, } from 'drizzle-orm/pg-core';
...
export const cooperativeStaff = pgTable('cooperative_staff', {
    id: uuid('id').defaultRandom().primaryKey(),
    cooperativeId: uuid('cooperative_id').references(() => cooperatives.id, { onDelete: 'cascade' }).notNull(),
    userId: uuid('user_id').references(() => users.id, { onDelete: 'cascade' }).notNull(),
    phoneNumber: varchar('phone_number', { length: 30 }).notNull(),
